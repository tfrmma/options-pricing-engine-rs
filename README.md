...
- Rough Bergomi (`rbergomi.rs`, `mc_rough_bergomi` in `mc.rs`) prices European, Asian, and up-and-out barrier options, is validated against an independent exact-covariance reference, and bootstraps its forward variance curve from ATM quotes. No calibration surrogate yet (dataset generator exists, training/inference don't), see [Rough Bergomi (work in progress)](#rough-bergomi-work-in-progress) for exactly what exists and what's still missing.
