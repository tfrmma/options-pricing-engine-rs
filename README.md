...
**Batch pricing a strike chain.** `batch_rough_bergomi` (`mc.rs`, re-exported from `batch.rs`) prices a whole chain of strikes (same spot/expiry/rate/div_yield/params/curve, the realistic case: one expiry, many strikes) against *one* simulated batch of paths, not one independent `mc_rough_bergomi` call per strike. Two things get shared that calling `mc_rough_bergomi` in a loop would rebuild every time: the setup (Σ/Cholesky/FFT plan/Γ, already the expensive part) and the price path itself, which doesn't depend on strike at all, only `payoff.eval` does, so `run_rbergomi_path` got split into a `simulate_rbergomi_price_path` step (the O(n_steps) Euler loop) and a thin per-payoff evaluation on top of it. Exact-match tested against `mc_rough_bergomi` with the same config and seed, not just close within MC noise: the path simulation never looks at strike, so a single-strike chain has to reproduce the scalar call bit for bit, and every strike in a multi-strike chain has to match its own independent scalar call the same way. Measured, not guessed: a 15-strike chain at 100k paths came out 15.9x faster batched than the same chain priced via 15 independent `mc_rough_bergomi` calls (`batch_rbergomi_is_faster_than_looping_scalar_calls`, `#[ignore]`d like the other timing benchmarks in this repo since it measures wall clock, not correctness, run it explicitly to reproduce). Only `European` for now, `AsianArithmetic`/`UpAndOut` would share the same `simulate_rbergomi_price_path` output but batching them isn't wired up yet, a smaller, separate piece of work.
...
- Rough Bergomi (`rbergomi.rs`, `mc_rough_bergomi` in `mc.rs`) prices European, Asian, and up-and-out barrier options, is validated against an independent exact-covariance reference, bootstraps its forward variance curve from ATM quotes, and has a batch strike-chain pricer for European (`batch_rough_bergomi`, 15.9x faster than looping scalar calls on a 15-strike chain). No calibration surrogate yet (dataset generator exists, training/inference don't), and batching isn't wired up for Asian/UpAndOut yet, see [Rough Bergomi (work in progress)](#rough-bergomi-work-in-progress) for exactly what exists and what's still missing.
