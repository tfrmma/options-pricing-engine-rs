...
- Multistart calibration is repeated local optimization, not a real global optimizer. More restarts help but don't guarantee the global minimum, and every restart still costs a full LM run, there's no early-exit sharing of information between restarts.
...
- No CI configured. 63 passing local tests is not the same guarantee as a required check on every PR.
