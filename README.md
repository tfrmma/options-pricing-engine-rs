...
| Rough Bergomi (Bayer, Friz, Gatheral 2016) | Monte Carlo via the hybrid scheme (Bennedsen, Lunde, Pakkanen 2017), κ=2, FFT tail convolution | European, Asian (arithmetic), up-and-out barrier, all validated, see [Rough Bergomi](#rough-bergomi-work-in-progress) |
| Deribit inverse (coin-settled) | Closed form (Deribit's own forward-based BS formula) plus `mc_result_to_coin`, a rescale bridge onto any Heston/Bates/rBergomi MC price | Full analytic Greeks (Δ, Γ, ν, Θ, vanna, volga) from the closed form; coin-settled smile pricing via the MC bridge; `implied_vol_coin` inverts a coin-denominated market price back to vol; `batch_price_coin`/`batch_greeks_coin`/`batch_implied_vol_coin` batch all three across a chain, see [Deribit inverse options](#deribit-inverse-coin-settled-options) |
...
`mc_rough_bergomi` in `mc.rs` is a real, usable pricer now: hybrid scheme (κ=2) for the near-kernel Wiener integrals via the Σ/Cholesky from commit 1, FFT tail convolution (`realfft`) for the far Riemann sum, mapped through the rBergomi variance formula (Bayer, Friz, Gatheral 2016, section 3.3) against a `ForwardVarianceCurve`, Euler-integrated log-price sharing the same `McConfig`/`Payoff`/`McResult` types as the Heston/Bates Monte Carlo pricer. It's now validated against an independent reference, not just its own internal consistency, see below. `European`, `AsianArithmetic`, and `UpAndOut` are all validated: the pathwise inequalities (Asian cheaper than European, up-and-out cheaper than vanilla) hold, and more importantly, `run_rbergomi_path`'s `running_sum`/`running_max` accumulation (shared code with `European`, which never reads either value, so a bug there could have sat invisible behind passing `European` tests indefinitely) matches Heston's independently-implemented accumulator to within MC noise in the vol-of-vol-collapses-to-flat-vol limit, not just the same ordering, the same *value*.

`ForwardVarianceCurve` and `RoughBergomiParams` (`types.rs`) are the two public types. The forward variance curve is piecewise-constant between listed expiries, deliberately not flat: it's arb-free by construction, not by a repair pass, strictly positive variance times strictly ascending expiries makes cumulative total variance strictly increasing, no calendar-arb check needed the way `local_vol.rs` needs one for the full (K,T) surface, this is 1D and ATM-only, there's no butterfly dimension to fight. `bootstrap_forward_variance_curve` (`rbergomi.rs`) builds one from ATM implied vol quotes: total variance at each expiry is `atm_vol²·T`, ξ0 per bucket is the increase in total variance divided by the bucket width. If the input quotes are themselves calendar-arb-free this comes out arb-free automatically; if they're not (crossed or stale quotes), it returns `Err(CurveBootstrapError)` naming the offending expiry rather than clamping or repairing it. That's a deliberately different philosophy from `local_vol.rs`'s `check_and_repair_surface`, which actively fixes what it finds, a full 2D surface has more than one plausible repair, a 1D calendar clash between two ATM quotes doesn't, there's no non-arbitrary number to invent, only a data problem to report back to whoever owns the feed.
...
**Batch pricing a payoff chain.** `batch_rough_bergomi` (`mc.rs`, re-exported from `batch.rs`) prices a whole chain of payoffs (same spot/expiry/rate/div_yield/params/curve, the realistic case: one expiry, many strikes, calls and puts and path-dependent payoffs mixed freely) against *one* simulated batch of paths, not one independent `mc_rough_bergomi` call per payoff. Two things get shared that calling `mc_rough_bergomi` in a loop would rebuild every time: the setup (Σ/Cholesky/FFT plan/Γ, already the expensive part) and the price path itself, which doesn't depend on the payoff at all, only `payoff.eval` does, so `run_rbergomi_path` got split into a `simulate_rbergomi_price_path` step (the O(n_steps) Euler loop) and a thin per-payoff evaluation on top of it. First version only took European strikes (`&[(f64, OptionType)]`), an artificial restriction: `simulate_rbergomi_price_path` already computes `running_sum`/`running_max` unconditionally regardless of what the payoff needs, same as the scalar path always has, so generalizing to the full `Payoff` enum (`&[Payoff]`) cost nothing, `AsianArithmetic` and `UpAndOut` ride along for free. Exact-match tested against `mc_rough_bergomi` with the same config and seed, not just close within MC noise, across a chain mixing all three payoff kinds: the path simulation never looks at the payoff, so every entry has to reproduce its own independent scalar call bit for bit regardless of what else is in the chain. Measured, not guessed: a 15-strike European chain at 100k paths came out ~15x faster batched than the same chain priced via 15 independent `mc_rough_bergomi` calls (`batch_rbergomi_is_faster_than_looping_scalar_calls`, `#[ignore]`d like the other timing benchmarks in this repo since it measures wall clock, not correctness, run it explicitly to reproduce).

**Validation against an independent reference.** Every test before this commit checked the hybrid+FFT scheme against *itself*, its own Σ reconstructs correctly, its own convolution matches a direct sum, its own Var(Yn) matches its own definition. None of that rules out the scheme correctly implementing the wrong thing. `rbergomi_fast_scheme_matches_exact_covariance_reference` (`mc.rs`) prices against a second, independent simulator built from the *true continuous-time process's own covariance* (`exact_joint_covariance`, `rbergomi.rs`, test-only): Y(t_i) and the driving Brownian increments sampled jointly from a closed-form covariance derived the same way as Σ (Lemma 4.3, same `hyp2f1`), no hybrid scheme, no FFT, no discretization approximation beyond the Cholesky factorization's own float error, O((2m)³) to factor so it doesn't scale, which is fine, it's a benchmark, not a production path, same role as the paper's own Figure 4. Fast and exact prices agree at three strikes to z=1.5-2.1 on their combined standard error. `rbergomi_short_maturity_skew_explodes_like_the_theory_predicts` checks the one property rough vol exists for: ATM skew exploding as T→0 like T^(H-1/2), not a tight exponent fit (a finite-h secant at finite T against a T→0 asymptotic result has real slack of its own), a generous ratio band between two maturities 8x apart, measured ratio 2.53 against a theory-predicted 2.45.

Building the exact reference caught two real bugs, neither found by inspection:
...
Still missing: the actual training step (offline, Python/PyTorch, not this repo) and the hand-rolled Rust inference layer with golden-value tests against the trained weights.
...
Forward-based, not spot-based: `price_coin`/`greeks` take the future/perp mark price directly as `forward`, the same way a trader actually quotes and hedges these, not `spot` + carry. `price_coin`/`greeks` are a closed-form, single-vol-input pricer, no smile.

**Bridging to the stochastic-vol MC engine.** `mc_result_to_coin` converts an existing USD-settled MC price (`mc.rs`, any of Heston/Bates/rough Bergomi, any of `European`/`AsianArithmetic`/`UpAndOut`) into the coin-settled equivalent, so a Heston or rBergomi smile can be priced in coin terms without a second, model-specific inverse pricer for each model. The first version of this divided the per-path payoff by the terminal price (`(S_T-K)^+/S_T`, path by path) and skipped the usual discount factor, matching the payoff's own definition at expiry. It compiled, ran, and was wrong: cross-checking it against `price_coin` (near-zero vol-of-vol collapsing both rBergomi and Heston MC toward flat BSM, same trick as the model-collapse tests elsewhere in this README) came back with z-scores in the 40s-60s, not MC noise, a real bug. Dividing by the *terminal, path-dependent* price silently prices under the wrong probability measure: a coin-settled payoff needs the expectation under the *share numeraire* measure (Q^S, numeraire = the asset itself), not the standard risk-neutral measure (Q, money-market numeraire) that the rest of this engine already uses correctly. Working out the Radon-Nikodym derivative between the two (`dQ^S/dQ = S_T·e^{-(r-q)T}/S_0`, a general change-of-numeraire result, not specific to flat vol) shows the S_T in the payoff's denominator cancels exactly against the S_T in that derivative, leaving:

```
E^{Q^S}[h(path)/S_T] = (e^{q·T}/S_0) · E^Q[h(path)]
```

for any USD payoff `h(path)`, European or path-dependent. In words: the coin price is just the *already-computed, already-discounted* USD MC price rescaled by a deterministic constant, `e^{qT}/S_0`. No new simulation, no drift change, no per-payoff special case, `mc_result_to_coin` is a five-line function. Verified two ways: algebraically against `price_coin`'s own closed form (worked out by hand, confirmed to 6 decimal places against a plain Black-Scholes calculation before touching the MC code at all), and against both rBergomi and Heston MC independently in the near-zero-vol-of-vol limit (`rbergomi_rescale_matches_closed_form_when_vol_of_vol_is_tiny`, `heston_rescale_matches_closed_form_when_vol_of_vol_is_tiny`), both agreeing with the closed form well inside MC noise this time.

Whether the coin-settled smile should ever differ from the direct-quoted USD smile for the same underlying is a separate question this doesn't answer, IV is IV regardless of settlement currency in principle, only the payoff/numeraire conversion changes, but a production system reconciling against Deribit's own quoted coin IVs would want to check that assumption against real market data rather than trust it by construction.

**Coin-denominated IV.** `implied_vol_coin` inverts a coin-denominated market price back to vol, same Halley-with-bisection-fallback architecture as `iv.rs` (same tolerance, same 10-iteration cap), forward-based since that's what a coin-settled quote actually gives you, and reuses `greeks()` for vega/volga in the Halley step instead of a second copy of that math. One asymmetry worth knowing before trusting a coin quote: a coin call is bounded above by 1 (the payoff `(S_T-K)^+/S_T` is bounded by 1 pointwise for any K>0, checked numerically across 200k random (F,K,vol,T) draws before relying on it, `price_in_bounds` rejects a call quoted at or above 1 the same way it rejects a price below intrinsic), but a coin put has no such bound, `(K-S_T)^+/S_T` is genuinely unbounded as S_T→0 and a deep ITM put can price well past 1 in coin terms, a real structural difference from linear puts (bounded by K in USD), not a bug to symmetrize away.

Picking test parameters for that surfaced a genuine ill-posedness, not a solver bug: a put deep enough ITM relative to a short expiry (K/F=60, 30 days) prices at intrinsic to six decimal places across the *entire* vol=0.05-3.0 range, vega is effectively zero there, and Halley has no gradient to work with. The solver still returns *a* root, just not necessarily the one that generated the test price (any vol in that range reprices to the same number within tolerance, they're observationally indistinguishable). `coin_iv_near_zero_vega_returns_a_self_consistent_root_not_necessarily_the_generating_one` checks exactly that: not which vol comes back, only that whatever does reprices consistently, or that the solver bails with `None` instead of returning a confident wrong answer, either outcome is acceptable in a region this flat.

**Batch pricing.** `batch_price_coin`/`batch_greeks_coin`/`batch_implied_vol_coin` (`batch.rs`) parallelize across a chain the same simple way `batch_heston`/`batch_bates` do, `contracts.par_iter().map(...)`, no shared-setup concern the way `batch_rough_bergomi` has: `price_coin`/`greeks` are closed-form and cheap, there's no expensive per-call object (no Σ, no Cholesky, no FFT plan) to hoist out of a loop. `InverseContract` (`types.rs`) is the forward-based analogue of `OptionContract` for this, and `CoinIvProblem` (also `types.rs`, moved there from `deribit_inverse.rs` in this same pass) pairs with it the same way `IvProblem` pairs with `OptionContract`, a placement inconsistency from when `CoinIvProblem` was first added, fixed here rather than left to diverge further.
...
| `rbergomi` | 13 | Hybrid scheme covariance Σ matches the Itô isometry at Σ₁,₁, symmetric, Cholesky reconstructs it exactly, stays positive definite down to α=-0.49 (H≈0.01, closer to the crypto short-dated regime than the paper's own H=0.07 test case), scales with n at the rate the paper predicts, optimal evaluation points b*_k land inside their cell, the 2F1 series matches an independent singularity-regularized quadrature, the FFT tail convolution matches an independent direct O(N²) sum, Var(Yn(t)) matches a closed form derived from the scheme's own definition (see [Rough Bergomi](#rough-bergomi-work-in-progress)), curve bootstrap recovers a flat curve and a known non-flat term structure exactly, rejects a genuinely crossed quote pair with the right expiry and sign, rejects a duplicate expiry |
| `mc` | 17 | European MC matches analytic Heston/Bates within a z-score bound on the MC's own standard error (both schemes), Asian call cheaper than European (real inequality, Heston and rBergomi both), up-and-out cheaper than vanilla with zero rebate (real inequality, Heston and rBergomi both), QE cuts bias ~20x vs Euler in a Feller-violating coarse-step regime, Poisson sampler mean check, rBergomi forward is a martingale (strike=0 call = E[disc·S_T] = S_0 exactly, by construction) within a z-score bound, rBergomi converges to Black-Scholes as η→0, rBergomi fast (hybrid+FFT) prices match an independent exact-continuous-covariance reference within a z-score bound across three strikes, short-maturity ATM skew ratio (T=0.02 vs T=0.16) lands within a generous band of the theory-predicted T^(H-1/2) ratio, rBergomi's Asian and up-and-out prices match Heston's independently-implemented path accumulator within a z-score bound in the vol-of-vol-collapses-to-flat-vol limit (not just the same ordering as the pathwise inequality, the same value), `batch_rough_bergomi` matches `mc_rough_bergomi` bit for bit (same RNG draws regardless of chain length or composition) for a single-payoff chain and for a chain mixing European (call and put), Asian, and up-and-out together |
| `bates` | 6 | Recovers Heston when jump intensity is zero, put-call parity, sign checks, no-static-arbitrage |
| `batch` | 9 | Batch price matches scalar calls, batch IV round-trips, batch Heston/Bates Greeks match scalar `PricingResult` field-by-field, batch price-only output agrees with batch Greeks output, same four checks (price/greeks/round-trip/price-vs-greeks agreement) repeated for `batch_price_coin`/`batch_greeks_coin`/`batch_implied_vol_coin` |
...
| `deribit_inverse` | 25 | Put-call parity in coin terms, delta/gamma/vega/theta match finite differences, vanna matches FD two independent ways (via delta and via vega), volga matches FD, call/put equality for vega/theta/vanna/volga (all four are parity-invariant, C-P=1-K/F doesn't depend on vol or T), ATM coin delta is order `1/F` not order 1, intrinsic value at expiry is `(F-K)/F` not `F-K`, vanna/volga are exactly zero past expiry, `mc_result_to_coin` rescale matches the closed form against both rBergomi and Heston MC independently in the vol-of-vol-collapses-to-BSM limit (the test that caught the first, wrong implementation, z-scores in the 40s-60s), `implied_vol_coin` round-trips at ATM/OTM/ITM/low-vol, rejects a bad or sub-intrinsic price, rejects a call priced at or above 1 (the pointwise payoff bound), accepts a put priced above 1 (no such bound for puts), returns a self-consistent (if not necessarily unique) root in a near-zero-vega deep-ITM-put regime instead of a confident wrong answer, see [Deribit inverse options](#deribit-inverse-coin-settled-options) |
...
- Rough Bergomi (`rbergomi.rs`, `mc_rough_bergomi` in `mc.rs`) prices European, Asian, and up-and-out barrier options, is validated against an independent exact-covariance reference, bootstraps its forward variance curve from ATM quotes, and has a batch payoff-chain pricer (`batch_rough_bergomi`, any mix of European/Asian/up-and-out, ~15x faster than looping scalar calls on a 15-strike chain). No calibration surrogate yet (dataset generator exists, training/inference don't), see [Rough Bergomi (work in progress)](#rough-bergomi-work-in-progress) for exactly what exists and what's still missing.
