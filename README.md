...
Still missing: the deep-calibration surrogate (offline-trained tiny MLP, hand-rolled Rust inference, no ONNX runtime for a ~5.6k-parameter network), bootstrapping `ForwardVarianceCurve` from market quotes, and `AsianArithmetic`/`UpAndOut` validation against a rough-vol path specifically.
...
- Rough Bergomi (`rbergomi.rs`, `mc_rough_bergomi` in `mc.rs`) prices vanilla European options and is now validated against an independent exact-covariance reference (not just its own internal consistency), no calibration surrogate yet, no curve bootstrap from market quotes, and `AsianArithmetic`/`UpAndOut` are untested against it, see [Rough Bergomi (work in progress)](#rough-bergomi-work-in-progress) for exactly what exists and what's still missing.
